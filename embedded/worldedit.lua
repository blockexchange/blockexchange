...
local function deserialize_workaround(content)
	...
	if not jit then
		...
		nodes = {}
		content = content:gsub("^%s*return%s*{", "", 1):gsub("}%s*$", "", 1)
		-- remove the starting and ending values to leave only the node data
		-- remove string contents strings while preserving their length
		local escaped = content:gsub("\\\\", "@@"):gsub("\\\"", "@@"):gsub("(\"[^\"]*\")", function(s)
			return string.rep("@", #s)
		end)
...
local function load_schematic(value)
	local version, content = worldedit.read_header(value)
	local nodes = {}
